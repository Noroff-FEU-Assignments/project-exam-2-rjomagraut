...
export default function AutoCompleteAdmin({ register }) {
          ...
          {filteredMedia.length > 0 ? (
            filteredMedia.map((post) => {
              return (
					...
					<Dropdown.Item href={`/admin/accommodations/edit/${post.id}`} >{post.hotel_name}</Dropdown.Item>
					  );
            })
          ) : (
...
AutoCompleteAdmin.propTypes = {
	register: PropTypes.func,
};

AutoCompleteAdmin.defaultProps = {
	register: () => {},
};
